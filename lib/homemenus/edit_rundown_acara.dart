...
void main() {
  runApp(EditRundownAcara(null, null, null));
}

class EditRundownAcara extends StatefulWidget {
  final context, string, rundown;
  EditRundownAcara(this.context, this.string, this.rundown);

  @override
  EditRundownAcaraState createState() => EditRundownAcaraState();
}

class EditRundownAcaraState extends State<EditRundownAcara> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  void initData() async {
    setState(() {
      Global.CURRENT_SCREEN = "edit_rundown_acara";
      startTime = Jiffy(widget.rundown['date_start'].toString()).dateTime;
      endTime = Jiffy(widget.rundown['date_end'].toString()).dateTime;
      savedAudioPath = Global.USERDATA_URL+widget.rundown['voice_path'].toString();
      selectedPhotoPath = Global.USERDATA_URL+widget.rundown['photo'].toString();
    ...
    titleController.text = widget.rundown['activity_name'].toString();
    placeNameController.text = widget.rundown['place_name'].toString();
    panduanController.text = widget.rundown['panduan'].toString();
    laranganController.text = widget.rundown['larangan'].toString();
    doaController.text = widget.rundown['doa'].toString();
  }
    ...
    return WillPopScope(
        ...
        child: Scaffold(body: SafeArea(child: Column(
            children: [
              Container(
                  ...
                  child: Row(
                      ...
                      children: [
                        ...
                        Text(widget.string.text82, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                        Container(
                            width: 50, height: 50, child: GestureDetector(
                            behavior: HitTestBehavior.translucent,
                            onTap: () {
                              Global.confirm(context, widget.string, widget.string.confirmation,
                                  widget.string.text83, () async {
                                    await Global.showProgressDialog(context, widget.string.text84);
                                    Global.httpPost(widget.string, Uri.parse(Global.API_URL+"/user/delete_rundown"),
                                      body: <String, String>{
                                        "id": widget.rundown['id'].toString()
                            ...
                            child: Center(
                                child: Icon(Ionicons.trash, color: Colors.white, size: 20)
                            )
                        ))
                      ]
                  )
                              ...
                              children: [
                                ...
                                GestureDetector(
                                    ...
                                    child: Container(
                                        ...
                                        child: Center(
                                            child: selectedPhotoPath==null?Icon(Ionicons.image, color: Colors.white, size: 80):
                                            (isPhotoLocal?Image.file(File(selectedPhotoPath), width: 150, height: 150, fit: BoxFit.cover)
                                            :Image.network(selectedPhotoPath, width: 150, height: 150, fit: BoxFit.cover))
                                        )
                                    )
                                ),
                                SizedBox(height: 16),
                                                        ...
                                                        Container(
                                                            ...
                                                            child: Center(
                                                                child: Container(
                                                                    ...
                                                                    child: (() {
                                                                      if (isRecording) {
                                                                        return Icon(Ionicons.pause, color: Colors.black, size: 12);
                                                                      } else {
                                              ...
                                              children: [
                                                Container(width: 25, height: 20, child: GestureDetector(
                                                    ...
                                                    onTap: () async {
                                                      ...
                                                      if (isPlaying) {
                                                        await audioPlayer.play(savedAudioPath, isLocal: isAudioLocal);
                                                      } else {
                                        ...
                                        child: GestureDetector(
                                            ...
                                            onTap: () async {
                                              ...
                                              try {
                                                ...
                                                request.files.add(await http.MultipartFile('photo', File(selectedPhotoPath).readAsBytes().asStream(), File(selectedPhotoPath).lengthSync(), filename: Uuid().v1() + ".jpg"));
              ...
              SizedBox(height: 10),
              BottomBar(context, widget.string, 1, "edit_rundown_acara")
            ]
        )))
    );
  }
