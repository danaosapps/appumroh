...
void main() {
  runApp(JuzVerses(null, null, null, null, null, null, null));
}

class JuzVerses extends StatefulWidget {
  final context, string, juzNumber, verseCount, lastChapter, lastVerse, isRead;
  JuzVerses(this.context, this.string, this.juzNumber, this.verseCount, this.lastChapter, this.lastVerse, this.isRead);

  @override
  JuzVersesState createState() => JuzVersesState();
}

class JuzVersesState extends State<JuzVerses> with WidgetsBindingObserver {
  ...
  var repeatType = 0;
  var verseSize = 17.0;
  var spellingSize = 10.0;
  var meaningSize = 10.0;
  var juzSize = 10.0;
  var lastChapter = 0;
  var lastVerse = 0;
  // 0 = no repeat
  ...
  void initData() async {
    setState(() {
      Global.CURRENT_SCREEN = "juz_verses";
      lastChapter = widget.lastChapter;
      lastVerse = widget.lastVerse;
    });
    var audioSpeed = await Global.readDouble("audio_speed", 1.0);
    setState(() {
      progressShown = true;
      player = AudioPlayer();
    ...
    Global.httpPost(widget.string, Uri.parse(Global.API_URL+"/user/get_verses_by_juz"),
        body: <String, String>{
          "juz": widget.juzNumber.toString()
        }, onSuccess: (response) {
          print("ALL VERSES:");
          print(response);
          setState(() {
          ...
          if (!widget.isRead) {
            for (var i = 0; i < verses.length; i++) {
              if (int.parse(verses[i]['chapter_id'].toString()) == lastChapter
                  && int.parse(verses[i]['verse_number'].toString()) == lastVerse) {
                setState(() {
                  currentAyatPosition = i;
                });
  ...
  void playNextAyat() {
    setState(() {
      ...
      lastChapter = int.parse(verses[currentAyatPosition]['chapter_id'].toString());
      lastVerse = int.parse(verses[currentAyatPosition]['verse_number'].toString());
    });
    Global.httpPost(widget.string, Uri.parse(Global.API_URL+"/user/update_last_verse"), body: <String, String>{
      "user_id": Global.USER_ID.toString(),
      "last_juz": widget.juzNumber.toString(),
      "last_chapter": lastChapter.toString(),
  ...
  void getAudios(reciterID, verseID) {
    Global.httpPost(widget.string, Uri.parse(Global.API_URL+"/user/get_audios_by_juz_number"),
        body: <String, String>{
          "juz": widget.juzNumber.toString()
        }, onSuccess: (response) {
          setState(() {
  ...
  bool isCurrentVerse(index) {
    if (int.parse(verses[index]['chapter_id'].toString()) == lastChapter
        && int.parse(verses[index]['verse_number'].toString()) == lastVerse) {
      return true;
    }
    return false;
  }
        ...
        child: Scaffold(body: SafeArea(child: Column(
            children: [
                  ...
                  child: Row(
                      ...
                      children: [
                        ...
                        Column(
                            ...
                            children: [
                              SizedBox(height: 8),
                              Text(widget.string.juz+" "+widget.juzNumber.toString(), style: TextStyle(
                                  color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold
                              )),
                              SizedBox(height: 2),
                              Text("( "+widget.verseCount.toString()+" "+widget.string.verses+" )",
                                ...
                                Row(
                                    children: [
                                      GestureDetector(
                                        ...
                                        onTap: () {
                                          setState(() {
                                            lastChapter = int.parse(verses[index]['chapter_id'].toString());
                                            lastVerse = int.parse(verses[index]['verse_number'].toString());
                                            currentAyatPosition = index;
                                          });
                                          Global.show(widget.string.text30);
                                          Global.httpPost(widget.string, Uri.parse(Global.API_URL+"/user/update_last_verse"), body: <String, String>{
                                            "user_id": Global.USER_ID.toString(),
                                            "last_juz": widget.juzNumber.toString(),
                                            "last_chapter": lastChapter.toString(),
                                        ...
                                        child: Padding(
                                          ...
                                          child: Icon(isCurrentVerse(index)?Ionicons.star:Ionicons.star_outline, color: Color(0xfff1c40f), size: 20)
                                        )
                                      ),
                                      Expanded(
                                          child: Container(
              ...
              Container(
                  width: width,
                  height: 50,
                  child: BottomBar(context, widget.string, 1, "juz_verses")
              )
            ]
        ))));
  }
